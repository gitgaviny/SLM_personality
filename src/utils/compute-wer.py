...
def default_cluster(word):
    unicode_names = [unicodedata.name(char, 'UNKNOWN') for char in word]
    ...
    for i in reversed(range(len(unicode_names))):
        ...
        if name.startswith('DIGIT'):  # 1
            unicode_names[i] = 'Number'
        elif (name.startswith('CJK UNIFIED IDEOGRAPH')
              or name.startswith('CJK COMPATIBILITY IDEOGRAPH')):
            unicode_names[i] = 'Mandarin'
        elif (name.startswith('LATIN CAPITAL LETTER')
              or name.startswith('LATIN SMALL LETTER')):
            unicode_names[i] = 'English'
        elif name.startswith('HIRAGANA LETTER'):
            unicode_names[i] = 'Japanese'
        elif (name.startswith('AMPERSAND')
              or name.startswith('APOSTROPHE')
              or name.startswith('COMMERCIAL AT')
              or name.startswith('DEGREE CELSIUS')
              or name.startswith('EQUALS SIGN')
              or name.startswith('FULL STOP')
              or name.startswith('HYPHEN-MINUS')
              or name.startswith('LOW LINE')
              or name.startswith('NUMBER SIGN')
              or name.startswith('PLUS SIGN')
              or name.startswith('SEMICOLON')):
            del unicode_names[i]
        else:
            return 'Other'
