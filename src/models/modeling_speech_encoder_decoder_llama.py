...
class WavLMAdapter(nn.Module):
    def __init__(self, config):
        ...
        self.layers = nn.ModuleList(WavLMAdapterLayer(config) for _ in range(3))
...
class SpeechEncoderDecoderModelLlama(PreTrainedModel, GenerationMixin_Instruct):
    ...
    def __init__(
        ...
        self.wavlm_adapter = WavLMAdapter(self.encoder.config)
    ...
    def forward(
        ...
        **kwargs,
        ...
        encoder_hidden_states = encoder_outputs[0]          # (B, T_feat, H)
        orig_seq_len = encoder_hidden_states.shape[1]

        if attention_mask is not None:
            orig_encoder_attention_mask = self.encoder._get_feature_vector_attention_mask(
                orig_seq_len, attention_mask
            )                                             # (B, T_feat)
        else:
            orig_encoder_attention_mask = None

        encoder_hidden_states = self.wavlm_adapter(encoder_hidden_states)  # (B, T_feat', H)
        new_seq_len = encoder_hidden_states.shape[1]
        ...
        if orig_encoder_attention_mask is not None:
            downsample_factor = orig_seq_len // new_seq_len
            if downsample_factor * new_seq_len != orig_seq_len:
                encoder_attention_mask = F.interpolate(
                    orig_encoder_attention_mask.unsqueeze(1).float(),  # (B, 1, T_feat)
                    size=new_seq_len,
                    mode="nearest",
                ).squeeze(1).to(dtype=orig_encoder_attention_mask.dtype)  # (B, T_feat')
            else:
                encoder_attention_mask = orig_encoder_attention_mask[:, ::downsample_factor]
        else:
        ...
        if (labels is not None) and (decoder_input_ids is None and decoder_inputs_embeds is None):
            ...
            labels[torch.arange(batch), first_pad_id] = eos_token_id = (
            ...
            if self.instruct:
                seq = labels[0]
                len_prompt = (
                    seq.eq(self.eosp_token_id).nonzero()[0] - seq.eq(self.bosp_token_id).nonzero()[0] - 1
                ).item()

                ignore_contents_mask = torch.full(
                    (batch, speech_len + len_prompt + 5),
                ...
                # A: build two masked label tensors from the same logits
                emo_pos = ignore_contents_mask.shape[1]
                labels_asr = labels.clone()
                labels_emotion = torch.full_like(labels, self.ignore_token_id)
                labels_emotion[:, emo_pos] = labels[:, emo_pos]
                labels_asr[:, emo_pos] = self.ignore_token_id
        ...
        if labels is not None:
            ...
            if labels_asr is not None and labels_emotion is not None:
                loss_fct = CrossEntropyLoss(ignore_index=self.ignore_token_id)
                loss_asr = loss_fct(
                    logits.reshape(-1, self.decoder.config.vocab_size),
                    labels_asr.reshape(-1),
                )
                loss_emotion = loss_fct(
                    logits.reshape(-1, self.decoder.config.vocab_size),
                    labels_emotion.reshape(-1),
                )
                if not hasattr(self.config, "asr_loss_weight") or self.config.asr_loss_weight is None:
                    raise ValueError(
                        "Missing required config field: `asr_loss_weight`. "
                        "Please set it in the model config (e.g., config.json)."
                    )
                if not hasattr(self.config, "emotion_loss_weight") or self.config.emotion_loss_weight is None:
                    raise ValueError(
                        "Missing required config field: `emotion_loss_weight`. "
                        "Please set it in the model config (e.g., config.json)."
                    )

                w_asr = float(self.config.asr_loss_weight)
                w_emo = float(self.config.emotion_loss_weight)
                loss = w_asr * loss_asr + w_emo * loss_emotion
            else:
